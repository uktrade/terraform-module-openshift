...
resource "aws_launch_configuration" "node-output" {
  ...
  security_groups = [
    ...
    "${aws_security_group.node-output.id}",
...
resource "aws_cloudwatch_metric_alarm" "node-output" {
  alarm_name = "${var.openshift["domain"]}-node-output"
  ...
  dimensions {
    AutoScalingGroupName = "${aws_autoscaling_group.node-output.name}"
  }
  alarm_actions = ["${aws_autoscaling_policy.node-output.arn}"]
}
