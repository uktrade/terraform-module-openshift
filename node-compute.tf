...
resource "aws_launch_configuration" "node-compute" {
  name_prefix = "${var.aws_conf["domain"]}-node-compute-"
  ...
  user_data = "${data.template_file.node-compute-cloudinit.rendered}"
...
resource "aws_autoscaling_group" "node-compute" {
  name = "${var.openshift["domain"]}-node-compute"
  launch_configuration = "${aws_launch_configuration.node-compute.name}"
  ...
  tag {
    key = "Name"
    value = "${var.openshift["domain"]}-node-compute"
...
resource "aws_autoscaling_policy" "node-compute" {
  name = "${var.openshift["domain"]}-node-compute"
  autoscaling_group_name = "${aws_autoscaling_group.node-compute.name}"
...
resource "aws_cloudwatch_metric_alarm" "node-compute" {
  ...
  dimensions {
    AutoScalingGroupName = "${aws_autoscaling_group.node-compute.name}"
  }
  alarm_actions = ["${aws_autoscaling_policy.node-compute.arn}"]
}
